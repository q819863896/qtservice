   ...
   <div class="login_box" :style="{height:$store.state.winH+'px'}">
      <el-form :model="loginInfo" :rules="rules2" ref="loginInfo" label-position="left" label-width="0px" class="login-container">
         ...
         <div class="password">
            ...
            <router-link to="/password?source=1"><span class="change">修改密码</span></router-link>
         </div>
         <div class="page_hint">首次登陆请点击修改密码进行密码初始化设置</div>
      ...
      <div class="page_info">本平台仅对签约客户开放，如有问题请联系你的老师</div>
   </div>
</template>
...
<script lang="ts">
   ...
   export default {
      ...
      methods: {
         ...
         toLogin() {
            this.$refs.loginInfo.validate((valid) => {
               if (this.backUrlData.autoLogin||valid) {
      ...
      mounted() {
         ...
         if (this.backUrlData.autoLogin === "true") {
            /*加载动画*/
